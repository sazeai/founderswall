...
export default function InPeriodLaunches() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(`/api/products?in_period=1&start=${start.toISOString()}&end=${end.toISOString()}`)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center justify-center">
            <div className="relative">
              <div className="animate-spin rounded-full h-12 w-12 border-4 border-orange-200 border-t-primary"></div>
              <div className="absolute inset-0 animate-ping rounded-full h-12 w-12 border-2 border-primary opacity-20"></div>
            </div>
            <span className="mt-4 text-lg font-medium text-foreground animate-pulse">
              Discovering amazing launches...
            </span>
          </div>
        </div>
      </section>
