...
import { Calendar, User, TrendingUp,ThumbsUp, Flame, Star, Zap } from "lucide-react"

export default async function ServerLaunchesSection() {
  // Get current launch period
  const { start, end } = getCurrentLaunchPeriod()
  // Fetch products (limit 12 for homepage)
  const { products, error } = await getProducts(12, 0)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product, index) => (
            ...
            >
              ...
              {/* Featured launch badge for top 1 */}
              {index < 1 && (
              ...
              <div className="flex items-start justify-between mb-6 pt-4">
                ...
                {/* Upvote count only, no button (for SEO) */}
                <div className="bg-gradient-to-r from-primary to-secondary text-white border-0 rounded-xl px-4 py-2 font-bold shadow-lg text-lg">
